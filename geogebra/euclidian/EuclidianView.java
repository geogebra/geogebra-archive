 ...
 * @version 
 */
public final class EuclidianView extends JPanel 
implements View, Printable, Transferable, ClipboardOwner {
    ...
    //  preview colors
     ...
     public static final Color colPreviewFill = 
                new Color(colDepPolygon.getRed(), 
                                 colDepPolygon.getGreen(),
                                 colDepPolygon.getBlue(),
                                 (int)(DEFAULT_POLYGON_ALPHA * 255));    

    // zoom rectangle colors                                
    private static final Color colZoomRectangle = new Color(200, 200, 250);
    private static final Color colZoomRectangleFill = new Color(200, 200, 250, 70);  

    // STROKES             
    private static MyBasicStroke standardStroke = new MyBasicStroke(1.0f);
    private static MyBasicStroke selStroke =
        new MyBasicStroke(1.0f + SELECTION_ADD);
    ...
    double xmin, xmax, ymin, ymax, invXscale, invYscale, xZero, yZero, xscale, yscale,
	       scaleRatio = 1.0; // ratio yscale / xscale 
    private double [] AxesTickInterval = {1,1}; // for axes   = axesNumberingDistances / 2    
    boolean showAxes = true;
    boolean showGrid = false;    
    private boolean antiAliasing = true;
    boolean showMouseCoords = false;
    private int pointCapturingMode = POINT_CAPTURING_OFF; // round point to grid point    
...
//  on add: change resetLists()
     ...
     */
    public EuclidianView(EuclidianController ec, boolean showAxes, boolean showGrid) {      
    ...
    private void initView(boolean repaint) {    	
        ...
        setCoordSystem(XZERO_STANDARD, YZERO_STANDARD, SCALE_STANDARD, SCALE_STANDARD, repaint);   
    }
    ...
    public static BasicStroke getStroke(float width, int type) {
        ...
        return new BasicStroke(
            width,
            endCap,
            standardStroke.getLineJoin(),
            standardStroke.getMiterLimit(),
            dash,
            0.0f);
    }
	...
	private void initCursor() {		
		...
		switch (mode) {
			case EuclidianView.MODE_ZOOM_IN:
				defaultCursor = getCursorForImage(app.getInternalImage("cursor_zoomin.gif"));
				break;
				
			case EuclidianView.MODE_ZOOM_OUT:
				defaultCursor = getCursorForImage(app.getInternalImage("cursor_zoomout.gif"));
				break;
		}
...
//			 Query for custom cursor support
    ...
    final public int toScreenCoordX(double xRW) {
        return 
        (int) Math.round(xZero + xRW * xscale);
    }
    ...
    final public int toScreenCoordY(double yRW) {
        return 
        (int) Math.round(yZero - yRW * yscale);
    }   
    ...
    /**         
     * convert real world coordinate x to screen coordinate x.
     * If the value is outside the screen it is clipped to one pixel
     * outside.
     * @param xRW
     * @return
     */
    ...
    /**
     * Sets real world coord system, 
     * where zero point has screen coords (xZero, yZero) and one unit
     * is xscale pixels wide on the x-Axis and yscale pixels heigh on the y-Axis.         
     */
    final public void setCoordSystem(double xZero, double yZero, double xscale, double yscale) {
        setCoordSystem(xZero, yZero, xscale, yscale, true);
    }
    
    /**
     * Sets real world coord system using min and max values for both axes in real world values.
     */
    final public void setRealWorldCoordSystem(double xmin, double xmax, double ymin, double ymax) {
    ...
    public void setCoordSystem(
		...
		double yscale,
        boolean repaint) {
        if (xscale < Kernel.MIN_PRECISION || xscale > 1E8) return;        
        if (yscale < Kernel.MIN_PRECISION || yscale > 1E8) return;
    ...
    public void updateSize() {
        ...
        bgImage = getGraphicsConfiguration().createCompatibleImage(width, height);
    ...
    private void setDrawMode(int mode) {
        if (mode != drawMode) {
            drawMode = mode;
            if (mode == DRAW_MODE_BACKGROUND_IMAGE)
                updateBackgroundImage();    
        }       
    }*/
    ...
    final private void setRealWorldBounds() {
        ...
        // tell kernel 
        kernel.setEuclidianViewBounds(xmin, xmax,
        		ymin, ymax, xscale, yscale);
    ...
    private void setAxesIntervals(double scale, int axis) {   
    	...
    	axesNumberFormat[axis].setMaximumFractionDigits(Math.max(-exp, kernel.getPrintDecimals()));
    ...
    /**
    * Returns xscale of this view. The scale is the number of pixels in screen space
    *  that represent one unit in user space.
    */
    ...
    /**
     * Returns the yscale of this view. The scale is the number of pixels in screen space
     *  that represent one unit in user space.
     */
	 ...
	 /**
	  * Returns the ratio yscale / xscale of this view. The scale is the number of pixels in screen space
	  *  that represent one unit in user space.
	  */
    ...
    */
    ...
    final public void paint(Graphics g) {       	
        ...
        /*
    	switch (drawMode) {
			case DRAW_MODE_BACKGROUND_IMAGE:     
                // draw background image (with axes and/or grid)
                if (bgImage == null) updateSize();
                else g2.drawImage(bgImage, 0,0, null);
                break;
            
	        default:
                 // DRAW_MODE_DIRECT_DRAW:
                drawBackground(g2, true);
		}	       */                             
    ...
    public int print(Graphics g, PageFormat pageFormat, int pageIndex) {
        if (pageIndex > 0)
            ...
            //  construction title
             ...
             StringBuffer sb = new StringBuffer(app.getPlain("ScaleInCentimeter"));
             if (printingScale <= 1) {
    			sb.append(": 1:");
            ...
            if (scaleRatio != 1.0) {
             ...
             if (line == null) line = sb.toString();
             else line = line + " - " + sb.toString();                       
     ...
     * @param forEPS: states if export should be optimized for eps. 
     * Note: if this is set to true, no traces are drawn.
     * 
     */ 
    ...
    private boolean isTracing() {
        ...
        while (it.hasNext()) {
            if (it.next().isTracing) return true;
        }
        return false;
    }
    ...
    /**
     * Returns image of drawing pad with specified width.
     * The height of the image is width * getHeight() / getWidth().
     * @param width
     * @return
     */
    ...
    /**
     * Returns image of drawing pad sized according to
    ...
    private BufferedImage createBufferedImage(int width, int height) throws OutOfMemoryError {
    	...
    	try {	       
    		...
    		try {	          		
        		...
        		System.err.println(e2.getMessage() + ": BufferedImage.TYPE_3BYTE_BGR");        		
        		System.gc();
	                ...
	                if (axesUnitLabels[1] != null) {
	                	if (piAxisUnit[1]) {	                		
	                		if (zero) {
	                			...
	                			strNum = axesNumberFormat[1].format(rw/Math.PI);
	                			if (strNum.equals("-1")) {
	                				sb.append("-");
	                			} else 	if (!strNum.equals("1")) { 	                					                		
	                				sb.append(strNum);
...
//    	 draw images
    ...
    final public GeoElement getLabelHit(Point p) {
        ...
        while (it.hasNext()) {
            ...
            if (d.hitLabel(p.x, p.y)) {
                ...
                if (geo.isEuclidianVisible()) return geo;
            }
        }
        return null;    
    }
    ...
    /**
     * returns array of GeoElements whose visual representation
     * is at screen coords (x,y). order: points, vectors, lines, conics
     */
    ...
    /**
     * returns array of independent GeoElements whose visual 
     * representation
    ...
    /**
        * returns array of changeable GeoElements out of
        * hits
        */
    ...
    /**
     * returns array of changeable GeoElements out of
     * hits that implement PointRotateable
     */
	 final public ArrayList getPointRotateableHits(ArrayList hits, GeoPoint rotCenter) {
    ...
    /**
        * returns array of  GeoElements of type geoclass whose visual 
        * representation
        * is at streen coords (x,y). order: points, vectors, lines, conics
        */
     ...
     */
	 final public ArrayList getOtherHits(ArrayList hits, Class geoclass, ArrayList result) {    	
	      return getHits(hits, geoclass, true, result);
	 }

 
	 final public ArrayList getHits(ArrayList hits, Class geoclass, ArrayList result) {
    ...
    /**
        * Stores all  GeoElements of type geoclass 
        * to result list. 
        * @param other == true: returns array of  GeoElements NOT of type geoclass 
        * out of hits. 
        */
    final private ArrayList getHits(ArrayList hits, Class geoclass, boolean other, ArrayList result) {
    	if (hits == null)
    		return null;
    ...
    /**
        * returns array of  GeoElements  whose visual 
        * representation is on top of
        * screen coords of Point p. If there are points at location p
        * only the points are returned. Otherwise
        * all GeoElements are returned.
    ...
    */
     ...
     */
    public final void zoom(double px, double py, double zoomFactor, boolean storeUndo) {
    	if (zoomer == null)
    		zoomer = new MyZoomer();    	
    ...
    /**
     * Zooms towards the given axes scale ratio. Note: Only the
     * y-axis is changed here. ratio = yscale / xscale;
     */
    ...
    /** 
     * Sets coord system of this view. 
     * Just like setCoordSystem but with previous animation.     
     * @param ox: x coord of new origin
     * @param oy: y coord of new origin
     * @param newscale
     */ 
    final void setAnimatedCoordSystem(double ox, double oy, 
                                     double newScale, boolean storeUndo) {                  
    ...
    private class MyZoomer implements ActionListener {
        ...
        public void init(double px, double py, double zoomFactor, boolean storeUndo) {
            this.px = px;
            this.py = py;
    ...
    private class MyAxesRatioZoomer implements ActionListener {
        ...
        public synchronized void actionPerformed(ActionEvent e) {
            counter++;    
            long time = System.currentTimeMillis() - startTime;
            if (counter == MyZoomer.STEPS || time > MyZoomer.MAX_TIME) { // end of animation             
                stopAnimation();
            } else { 
                factor = 1.0 + (counter * add) / oldScale;
    ...
    private class MyMover  implements ActionListener {          
        ...
        public synchronized void startAnimation() {     
            dx = xZero - ox;
            dy = yZero - oy;
            if (kernel.isZero(dx) && kernel.isZero(dy)) return;
