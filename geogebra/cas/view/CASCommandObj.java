 ...
 * @author James King
 * 
 * This class is for the CAS Command Object -- it manages such functions such
 * as: split(); <-- splits incoming strings into separate commands parse(); <--
 * parses commands and formats their text using GeoTDS. execute(); <-- runs the
 * command object set(); <-- changes the initial
 * 
 * for reference, regexp: "([^"\\]|\\.)*" matches all strings...
 */
...
public class CASCommandObj {
	...
	public void execute() {
		for (int i = 0; i < commands.size(); i++)
			responses.add(parentSession.evaluate(((GeoTDS) commands.get(i))
					.get()));
		hasExecuted = true;
