...
/**
 * Dialog which provides for exporting into an HTML page enriched with
 * an Applet.
 * 
...
public class WorksheetExportDialog extends JDialog {
    ...
    private JCheckBox showToolbarHelpCB;
    ...
    /**
     * The General Settings Tab contains some of the more general
     * settings.
     */
	...
	showFrame = new JCheckBox(app.getPlain("DoubleClickToOpen"));
	showFrame.setFont(app.getSmallFont());
	showFrame.setSelected(false);
	p = new JPanel(new BorderLayout());
	p.setBorder(BorderFactory.createEmptyBorder(0, 25, 10, 0));
	p.add(showFrame, BorderLayout.WEST);
	...
	showFrame.setEnabled(useWorksheet);
    }
	...
	if (useWorksheet) {
	    ...
	    sb.append(showFrame.isSelected());
	    sb.append("\">\n");
	}
	else {// button type
	...
	sb.append(showToolbarHelpCB.isSelected());
	sb.append("\">\n");
