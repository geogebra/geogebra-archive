...
!echo "NSIS Ultra Modern User Interface version 1.00 beta 2 - 2005-2009 SuperPat"
!echo "Based on: NSIS Modern User Interface version 1.8 - 2002-2009 Joost Verburg"
...
Var UMUI_INSTALLFLAG
; Contain a OR of all the flags define here:
...
;definition of all the other flags
...
; Taken from the NSIS Documentation
!macro UMUI_GETINSTALLERFULLPATH

  Push $R0

  System::Call 'kernel32::GetModuleFileNameA(i 0, t .R0, i 1024) i r1'
  ;$R0 will contain the installer filename
;  StrCpy $R0 $EXEPATH    ;Only compatible with NSIS version >= 2.26

  Exch $R0

!macroend

!macro UMUI_GETINSTALLERFILENAME

  Push $R0
  Push $R1

  !insertmacro UMUI_GETINSTALLERFULLPATH
  Pop $R0
...
;  StrCpy $R0 $EXEFILE    ;Only compatible with NSIS version >= 2.26
...
!macro UMUI_SAVEPARAMSTOREGISTRY
  ...
  !ifndef MUI_PAGE_UNINSTALLER
    ...
    IfAbort install_aborted${UMUI_UNIQUEID}
      ...
      ClearErrors
      ReadIniStr $R0 "$PLUGINSDIR\Registry.ini" "Settings" "number"
      IfErrors end${UMUI_UNIQUEID} 0

        StrCpy $R1 "0"
        loop${UMUI_UNIQUEID}:

          IntOp $R1 $R1 + 1
          IntCmp $R1 $R0 0 0 end${UMUI_UNIQUEID}
...
!macro UMUI_PATH_SAVE
  ...
  !ifdef UMUI_INSTALLERFULLPATH_REGISTRY_VALUENAME
    ...
    !else
      !insertmacro UMUI_GETINSTALLERFULLPATH
      Pop $MUI_TEMP1
      !insertmacro UMUI_ADDPARAMTOSAVETOREGISTRYKEY "${UMUI_INSTALLERFULLPATH_REGISTRY_ROOT}" "${UMUI_INSTALLERFULLPATH_REGISTRY_KEY}" "${UMUI_INSTALLERFULLPATH_REGISTRY_VALUENAME}" $MUI_TEMP1
    !endif
  !endif
  ...
  Function "${LEAVE}"
    ...
    !insertmacro UMUI_IF_INSTALLFLAG_ISNOT ${UMUI_CANCELLED}
      ...
      reloadinstall:
		...
		HideWindow
        !ifndef MUI_UNINSTALLER
            ExecWait "$R0 $R1 /L=$LANGUAGE /NCRC"
        !else
            ExecWait "$R0 $R1 /L=$LANGUAGE _?=$INSTDIR"
        !endif
        Quit
...
!macro UMUI_FUNCTION_INFORMATIONPAGE PRE LEAVE
  ...
  Function "${PRE}"
    ...
    !ifdef UMUI_INFORMATIONPAGE_USE_RICHTEXTFORMAT
      ...
      File "/oname=$PLUGINSDIR\Information.rtf" "${FILE}"

      !insertmacro MUI_PAGE_FUNCTION_CUSTOM PRE

      !insertmacro INSTALLOPTIONS_WRITE "Information.ini" "Field 2" Type "RichText"
      !insertmacro INSTALLOPTIONS_WRITE "Information.ini" "Field 2" State '$PLUGINSDIR\Information.rtf'

    !else

      File "/oname=$PLUGINSDIR\Information.txt" "${FILE}"
      ...
      ClearErrors
      FileOpen $MUI_TEMP1 "$PLUGINSDIR\Information.txt" r
      IfErrors 0 +2
        Abort
...
!macro UMUI_INSTALLOPTIONSEX_CONVERT FILE
  ...
  !ifdef UMUI_USE_INSTALLOPTIONSEX
    !ifndef UMUI_DONT_USE_IOEX_CONVERTER
      ...
      !define UMUI_UNIQUEID_CONVERT ${__LINE__}
      ...
      loop${UMUI_UNIQUEID_CONVERT}:
        ...
        StrCmp $R1 "Button" 0 inotButton
          ...
          Goto test${UMUI_UNIQUEID_CONVERT}
        inotButton:
        ;else if Link
      ...
      test${UMUI_UNIQUEID_CONVERT}:
      ;while $0 <= $R0
      IntCmp $0 $R0 0 loop${UMUI_UNIQUEID_CONVERT} 0

      !undef UMUI_UNIQUEID_CONVERT
...
!macro UMUI_INSTALLOPTIONSEX_CONVERT_NEXT FILE
  ...
  !ifdef UMUI_USE_INSTALLOPTIONSEX
    !ifndef UMUI_DONT_USE_IOEX_CONVERTER
      ...
      !define UMUI_UNIQUEID_CONVERTNEXT ${__LINE__}
      ...
      ;do
      loop${UMUI_UNIQUEID_CONVERTNEXT}:
        IntOp $0 $0 + 1

        !insertmacro MUI_INSTALLOPTIONS_READ $R1 "${FILE}" "Field $0" "Type"
        ;if Button
        StrCmp $R1 "Button" 0 test${UMUI_UNIQUEID_CONVERTNEXT}

          !insertmacro MUI_INSTALLOPTIONS_READ $8 "${FILE}" "Field $0" "RefFields"
          StrCmp $8 "" test${UMUI_UNIQUEID_CONVERTNEXT} 0
            !insertmacro MUI_INSTALLOPTIONS_READ $R8 "${FILE}" "Field $0" "HWND"
            !insertmacro MUI_INSTALLOPTIONS_WRITE "${FILE}" "Field $8" "HWND2" "$R8"

      test${UMUI_UNIQUEID_CONVERTNEXT}:
      ;while $0 <= $R0
      IntCmp $0 $R0 0 loop${UMUI_UNIQUEID_CONVERTNEXT} 0

      !undef UMUI_UNIQUEID_CONVERTNEXT
...
!macro INSTALLOPTIONS_DISPLAY_RETURN FILE
  ...
  ; we can't use InstallOptions::dialog function with UMUI because the background could not be initialised
  Push $MUI_TEMP1
  Push $MUI_TEMP2
  Push $R0
  Push $0
...
;-----
    ...
    test${UMUI_UNIQUEID}:
      IntOp $1 $1 + 1
    ;while $0 <= $R0
      IntCmp $0 $R0 0 loop${UMUI_UNIQUEID} 0
